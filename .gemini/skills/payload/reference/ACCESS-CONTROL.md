...
export const Posts: CollectionConfig = {
  ...
  access: {
    ...
    delete: async ({ req, id }) => {
      const hasComments = await req.payload.count({ collection: 'comments', where: { post: { equals: id } } });
...
export const Posts: CollectionConfig = {
  slug: 'posts',
  access: { create: authenticated, read: authenticatedOrPublished, update: adminsOrEditors, delete: admins },
...
const salaryField: NumberField = {
  name: 'salary',
  type: 'number',
  access: { read: salaryReadAccess, update: salaryUpdateAccess },
};
```
...
const arrayField: ArrayField = {
  ...
  fields: [
    ...
    { name: 'content', type: 'text', access: { read: contentReadAccess } },
  ],
};
```
...
const groupField: GroupField = {
  ...
  type: 'group',
  access: { read: internalOnlyAccess, update: internalOnlyAccess },
  fields: [
...
const tenantAccess: Access = ({ req: { user } }) => {
  ...
  return { tenant: { equals: (user as User).tenantId } };
};

export const Posts: CollectionConfig = {
  slug: 'posts',
  access: { create: tenantAccess, read: tenantAccess, update: tenantAccess, delete: tenantAccess },
  fields: [
    { name: 'title', type: 'text' },
    {
      name: 'tenant',
  ...
  fields: [
    {
      ...
      hasMany: true,
      options: ['admin', 'editor', 'user'],
      access: { update: preventSelfRoleChange },
    },
  ],
};
```
...
export const preventDeleteWithDependencies: Access = async ({ req, id }) => {
  ...
  const dependencyCount = await payload.count({ collection: 'related-items', where: { parent: { equals: id } } });
