...
O princípio do starter: a IA imita o padrão do `server/config.ts` em vez de reinventar.
...
```
server/config.ts ──(generateTypes no boot)──▶ server/types.ts  (Config)
                                                  ▼
...
- **`server/config.ts`** — o catálogo de capacidades do Payload (access, hooks, jobs,
  endpoints, fields). Ao adicionar collection/global/job/endpoint, siga o estilo dele.
  Leia os comentários: o avançado está explicado; o básico está só presente.
...
- Duas collections auth (`users`, `api-keys`) fazem `req.user` virar união
  (`User | ApiKey`) — faça **narrow por `collection`** (ex.: `asUser` em `config.ts`)
  antes de acessar `roles`/`email`.
