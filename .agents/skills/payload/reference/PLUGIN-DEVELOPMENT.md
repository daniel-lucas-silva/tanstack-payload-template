...
```json
{
  ...
  "exports": {
    ...
    "./types": { "import": "./dist/exports/types.js", "types": "./dist/exports/types.d.ts" }
  },
  "files": ["dist"],
  "scripts": {
    "build": "bun build ./src/index.ts --outdir dist --target bun --packages bundle",
    "build:types": "tsc --emitDeclarationOnly --outDir dist",
    "clean": "rimraf dist *.tsbuildinfo",
    "dev:generate-types": "bunx payload generate:types --config ./dev/payload.config.ts",
    "test": "bun test",
    "test:e2e": "playwright test",
    "lint": "oxlint",
    "prepublishOnly": "bun run clean && bun run build"
  },
  "dependencies": { "@payloadcms/translations": "^3.0.0" },
  "devDependencies": {
...
- Export entry point: main + `/types` (sem `/client` nem `/rsc` — projeto sem admin UI)
- Build com Bun (`bun build --target bun`), não SWC
- Dev scripts via Bun (`bunx payload`), sem Next.js
- Test scripts para integration (Vitest) e e2e (Playwright) tests
    ...
    return {
      ...
      collections: config.collections?.map((collection) => {
        if (options.collections?.includes(collection.slug)) {
          return { ...collection, fields: [...(collection.fields || []), ...seoFields] };
        }
        return collection;
      }),
    };
...
export const redirectsPlugin =
  ...
  (config: Config): Config => {
    ...
    return { ...config, collections: [...(config.collections || []), redirectsCollection] };
  };
```
...
const resaveChildrenHook: CollectionAfterChangeHook = async ({ doc, req, operation }) => {
  if (operation === 'update') {
    ...
    const children = await req.payload.find({ collection: 'pages', where: { parent: { equals: doc.id } } });

    for (const child of children.docs) {
      await req.payload.update({ collection: 'pages', id: child.id, data: child });
    }
  }
  return doc;
};
...
export const seoPlugin =
  ...
  (config: Config): Config => {
    ...
    return { ...config, endpoints: [...(config.endpoints ?? []), generateTitleEndpoint] };
  };
```
...
export const myPlugin =
  ...
  (config: Config): Config => {
    ...
    return {
      ...
      collections: config.collections?.map((collection) => {
        if (options.collections?.includes(collection.slug)) {
          return { ...collection, fields: [...(collection.fields || []), ...fields] };
        }
        return collection;
      }),
    };
...
export const myPlugin =
  ...
  (config: Config): Config => {
    ...
    config.collections.push({ slug: 'plugin-collection', fields: [{ name: 'title', type: 'text' }] });
    ...
    if (options.collections) {
      for (const collectionSlug of options.collections) {
        ...
        if (collection) {
          collection.fields.push({ name: 'addedByPlugin', type: 'text' });
        }
      }
    }
    ...
    config.endpoints = [
      ...(config.endpoints ?? []),
      { path: '/my-endpoint', method: 'get', handler: async () => Response.json({ message: 'Hello' }) },
    ];
...
export const translations = {
  en: { 'plugin-name:fieldLabel': 'Field Label', 'plugin-name:fieldDescription': 'Field description' },
  es: { 'plugin-name:fieldLabel': 'Etiqueta del campo', 'plugin-name:fieldDescription': 'Descripción del campo' },
};
...
export const myPlugin =
  ...
  (config: Config): Config => ({
    ...config,
    i18n: { ...config.i18n, translations: deepMergeSimple(translations, config.i18n?.translations ?? {}) },
  });
```
...
export const myPlugin =
  ...
  (config: Config): Config => {
    ...
    config.onInit = async (payload) => {
      ...
      if (totalDocs === 0) {
        await payload.create({ collection: 'plugin-collection', data: { id: 'seeded-by-plugin' } });
      }
    };
...
export const myPlugin =
  ...
  async (incomingConfig: Config): Promise<Config> => {
    ...
    const customCollection = await pluginConfig.collectionOverride?.({ defaultCollection });

    return { ...incomingConfig, collections: [...incomingConfig.collections, customCollection] };
  };
```
...
interface PluginConfig {
  products?: { collectionOverride?: CollectionOverride };
}
...
```ts
interface PluginConfig {
  sync: Array<{ collection: string; fields?: string[]; onSync?: (doc: any) => Promise<void> }>;
}
...
incomingConfig.typescript.schema.push((args) => {
  ...
  jsonSchema.properties.ecommerce = {
    type: 'object',
    properties: {
      collections: { type: 'object', properties: { products: { type: 'string' }, orders: { type: 'string' } } },
    },
  };
      ...
      async (args) => {
        ...
        if (status >= 500 || captureErrors.includes(status)) {
          captureException(error, {
            tags: { collection: args.collection?.slug, operation: args.operation },
  ...
  if (field.type === 'relationship') {
    return {
      ...
      filterOptions: ({ relationTo }) => {
        return { and: [field.filterOptions?.(relationTo) || {}, { tenant: { equals: req.user?.tenant } }] };
      },
    };
  }
  return field;
...
describe('Plugin integration tests', () => {
  test('should add field to collection', async () => {
    const post = await payload.create({ collection: 'posts', data: { title: 'Test', addedByPlugin: 'plugin value' } });
